...
thick_interp = np.zeros([1,coord_num*gclass])#Interpolated sample thickness data
...
#f#or i in range(len(gclass_label)):
    #plt.plot(x,thick_interp[0,coord_num * i:coord_num * (i+1)],label=gclass_label[i])
#plt.legend()
#plt.show()


# # Effect of non-ideal data set on inversion
...
for i in range(X_test.shape[0]):
    for j in range(gclass):
        f = interp1d(x,X_test_norm[i,j * coord_num : (j+1) * coord_num], kind="linear",bounds_error=False,fill_value='extrapolate')
...
a = pd.read_csv(
    '../FITTNUSS-forward_training_DNN/sendai_increased_class.csv', delimiter=',')
print(a)
y = pd.DataFrame()
output = []
...
with open('output_final_20_11_roi2000_TC_1.csv', 'w') as outfile:
    ...
    for index in range(len(a)):
        df = y.append(pd.read_csv(
            '../FITTNUSS-forward_training_DNN/sendai_increased_class.csv', skiprows=[index+1]))
        print(df)
        ...
        for j in range(gclass):
            # Interpolation function of jth granularity level
            f = interp1d(df['distance'], df.iloc[:, j+2],
